    ...
    if (data.tasks && data.tasks.length !== 0) {
        ...
        data.tasks.forEach(task => {
            html +=
                ...
                `<h2>${task.name}</h2>` +
                `<data-label="Category"><select>${categoriesOptions}</select>` +
                `<data-label="Status"><select>
        <option value="not-started" ${task.status === 'Not Started' ? 'selected' : ''}>Not Started</option>
         <option value="in-progress" ${task.status === 'In Progress' ? 'selected' : ''}>In Progress</option>
       <option value="completed" ${task.status === 'Completed' ? 'selected' : ''}>Completed</option>
     </select>` +
                `<data-label="Due Date"><input type="date" value="${task.dueDate}">` +
                `<data-label="Est. Time"><input type="number" value="${task.estimatedTime}" min="0"> min` +
                `<data-label="Spent">${task.timeSpent} min</data-label>` +
